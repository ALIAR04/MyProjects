...
int main() {
    InitWindow(WIDTH_OF_SCREEN, HEIGHT_OF_SCREEN, "ASM");       //name of window
    ...
    while (!WindowShouldClose()) {
        ...
        if (is_moving) {
            ...
            if (type_moving == 2) {
                ...
                float temp = get_remainder_asm(ball_position.x * 0.03);
                if (temp > 3.14) temp -= 6.28;
                else if (temp < -3.14) temp += 6.28;
                
                ball_position.y = sinouside_move_asm(temp, y0);
