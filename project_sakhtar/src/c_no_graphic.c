...
void straight_moving(float *x_position, float x_speed) {
    *x_position += x_speed;
}

void mohadab_moving(float *y_position, float *y_speed) {
    *y_position += *y_speed;
    *y_speed += BALL_GRAVITY;
}

void sinouside_moving(float *y_position,float x_position, float y0) {
    float temp = remainder(x_position * 0.03 , 6.2831);
    if (temp < -3.14) temp = - (temp + 3.14);
    else if (temp > 3.14) temp = - (temp - 3.14);
    
    *y_position = y0 + 35 * sin(temp);
}
...
int main() {
    ...
    while (1) {
        ...
        if (is_moving) {
            ...
            straight_moving(&x_position, x_speed);
            if (type_moving == 2) {
                sinouside_moving(&y_position, x_position, y0);
            } else if (type_moving == 3) {
                mohadab_moving(&y_position, &y_speed);
            }
        }
